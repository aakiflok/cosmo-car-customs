...
export default function PackageTable({ packages }: PackageTableProps) {
  return (
    <section className="w-full bg-elevated section-pad border-t border-hairline">
      <div className="w-full px-6 sm:px-10 lg:px-16">
        ...
        <div className="grid gap-px bg-hairline md:grid-cols-3">
          {packages.map((pkg, idx) => {
            const isConcours = pkg.tier === 'concours';
            return (
              <div
                key={idx}
                className={`relative flex flex-col p-8 transition-all ${
                  isConcours
                    ? 'bg-[#141008] outline outline-1 outline-[#C79A3B] z-10'
                    : pkg.highlight
              ...
              >
                {/* Tier badge */}
                {isConcours && (
                  <div className="absolute top-0 left-8 -translate-y-1/2 bg-[#C79A3B] px-3 py-1">
                    <span className="label-uc text-[8px] text-black">Concours</span>
                  </div>
                )}
                {!isConcours && pkg.highlight && (
                ...
                <div className="mb-8">
                  <div className={`label-uc text-[9px] mb-3 ${
                    isConcours ? 'text-gold' : 'text-white/40'
                  }`}>
                    {pkg.tier === 'essential' ? 'Tier 01' : pkg.tier === 'signature' ? 'Tier 02' : 'Tier 03'}
                  </div>
                  <h3 className="display-sm text-white mb-4">{pkg.name}</h3>
                  <div className={`h-px w-8 ${ isConcours ? 'bg-gold' : 'bg-rossa' }`} />
                </div>
                ...
                <ul className="space-y-3 mb-10 flex-1">
                  {pkg.features.map((feature, fidx) => (
                    <li key={fidx} className="flex items-start gap-3">
                      <Check
                        size={13}
                        className={`flex-shrink-0 mt-[3px] ${ isConcours ? 'text-gold' : 'text-rossa' }`}
                        aria-hidden="true"
                      />
                ...
                <Link
                  ...
                  className={`w-full py-3 text-center label-uc text-[9px] border transition-all block ${
                    isConcours
                      ? 'bg-[#C79A3B] border-[#C79A3B] text-black hover:bg-[#b5872e]'
                      : pkg.highlight
